...
function collectionWhere(param: string) {
  return UUID_RE.test(param)
    ? eq(collections.id, param)
    : eq(collections.slug, param);
}

app.get("/collections", async (c) => {
  const cursor = c.req.query("cursor");
  const where = cursor
    ? and(eq(collections.public, true), lt(collections.id, cursor))
  ...
  const rows = await db
    .select({
      ...
      ownerName: users.name,
      ownerImage: users.image,
      listCount: sql<number>`cast((select count(*) from ${collectionLists} where ${collectionLists.collectionId} = ${collections.id}) as int)`,
    })
    .from(collections)
  ...
  const nextCursor =
    rows.length === COLLECTIONS_PAGE_SIZE ? rows[rows.length - 1].id : null;
  return c.json({ items: rows, nextCursor });
});
...
app.get("/collections/:collectionId", async (c) => {
  const collection = await db.query.collections.findFirst({
    where: collectionWhere(c.req.param("collectionId")),
  });
  ...
  const owner = await db.query.users.findFirst({
    where: eq(users.id, collection.ownerId),
    columns: { id: true, name: true, image: true },
  });
  const listRows = await db
    ...
    .from(collectionLists)
    .innerJoin(lists, eq(lists.id, collectionLists.listId))
    .where(eq(collectionLists.collectionId, collection.id))
    .orderBy(asc(collectionLists.position), asc(collectionLists.createdAt));
  return c.json({
    id: collection.id,
...
app.post(
  ...
  async (c) => {
    ...
    const userId = authUser?.session?.user?.id;
    if (!userId) return c.json({ error: "Unauthorized" }, 401);
    const { name, description, public: isPublic } = c.req.valid("json");
    const baseSlug = slugify(name);
    let created: typeof collections.$inferSelect | undefined;
    for (let attempt = 0; attempt < 6; attempt++) {
      const slug =
        baseSlug.length === 0
          ? null
...
app.delete("/collections/:collectionId", async (c) => {
  ...
  if (!collection) return c.json({ error: "Not found" }, 404);
  if (collection.ownerId !== userId) return c.json({ error: "Forbidden" }, 403);
...
app.post(
  ...
  zValidator("json", z.object({ listId: z.string().uuid() })),
  async (c) => {
    const authUser = getOptionalUser(c);
    const userId = authUser?.session?.user?.id;
    if (!userId) return c.json({ error: "Unauthorized" }, 401);
    ...
    const [maxRow] = await db
      .select({ pos: max(collectionLists.position) })
      .from(collectionLists)
      .where(eq(collectionLists.collectionId, collection.id));
    await db
      .insert(collectionLists)
      .values({
...
app.delete("/collections/:collectionId/lists/:listId", async (c) => {
  const authUser = getOptionalUser(c);
  const userId = authUser?.session?.user?.id;
  if (!userId) return c.json({ error: "Unauthorized" }, 401);
  const collection = await db.query.collections.findFirst({
  ...
  if (collection.ownerId !== userId) return c.json({ error: "Forbidden" }, 403);
  await db
    .delete(collectionLists)
    .where(
      and(
        eq(collectionLists.collectionId, collection.id),
        eq(collectionLists.listId, c.req.param("listId"))
      )
    );
  return c.body(null, 204);
});
